...
def main() -> None:
    out_dir = PROJECT_ROOT / "output" / "counting_experiment_val_new"
    pic_dir = PROJECT_ROOT / "图片"
    images_dir = PROJECT_ROOT / "all_datasets" / "cowDataset" / "images" / "val_new"
    model_path = PROJECT_ROOT / "runs" / "detect" / "cow_120_on_basecommon" / "weights" / "best.pt"
    pic_dir.mkdir(parents=True, exist_ok=True)

    with (out_dir / "yolov8_counting_val_new_results.csv").open(encoding="utf-8") as file:
        rows = list(csv.DictReader(file))

    # Representative mid/near-view examples selected from exact-count samples.
    sample_names = [
        "000000225480.jpg",
        "000000235547.jpg",
        "000000562092.jpg",
    ]
    row_by_name = {row["image_name"]: row for row in rows}
    samples = [row_by_name[name] for name in sample_names]
    ...
    example_path = out_dir / "yolov8_counting_examples_val_new.png"
    canvas.save(example_path)
    shutil.copyfile(example_path, pic_dir / "图4-X_YOLOv8牛只计数结果示例_val_new.png")
    ...
    plt.tight_layout()
    error_path = out_dir / "yolov8_count_error_distribution_val_new.png"
    plt.savefig(error_path)
    plt.close()
    shutil.copyfile(error_path, pic_dir / "图4-X_YOLOv8计数误差分布_val_new.png")
